/* Iconos del sistema KOKORO — replicando los Material Symbols usados en el
   Figma 104:487 (arrow_downward y arrow_outward). SVGs inline para no
   depender del servidor de Figma localhost:3845. */

export function ArrowDownward({ size = 24, className = '' }) {
  return (
    <svg
      width={size}
      height={size}
...
export function ArrowOutward({ size = 18, className = '' }) {
  return (
    <svg
      width={size}
      height={size}
...
/* Moon — variante para el toggle de tema en light mode (indica click→dark). */
...
/* Sun (estilo Phosphor regular) — para el toggle de tema en TopBar. */
...
/* Iconos para "La solución disruptiva" (Figma 137:194). Replican los
   Material Symbols Repeat, Tag e Intersect en stroke 1.5px currentColor. */
...
export function Tag({ size = 24, className = '' }) {
  return (
    <svg
      width={size}
      height={size}
...
/* Intersect — dos círculos solapados (estilo Venn) representando la
   unión entre bienestar físico y placer emocional. */
export function Intersect({ size = 24, className = '' }) {
  return (
    <svg
      width={size}
      height={size}
...
/* Chevrons direccionales — usados en la paginación del carrusel
   "Plan faseado" para navegar entre fases. Mantienen el grosor del resto
   del set (1.5px, currentColor). */
